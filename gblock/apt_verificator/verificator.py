...
def neighbours_indeces(i, j, layout):
    ...
    ns = {"u": [i, j + 1], "r": [i + 1, j], "d": [i, j - 1], "l": [i - 1, j]}
...
def check_corridor_neighbourhood(layout):
    ...
    for apt in apartments:
        ...
        for cell_coords in cells_flattened:
            i, j = cell_coords.index(glob=True)
            for n_coords in neighbours_indeces(i, j, layout):
                n_indeces.add(n_coords)
        cells_coords_set = set()
        for cell in cells_flattened:
            cells_coords_set.add(
                str(cell.index(True)[0]) + "," + str(cell.index(True)[1])
            )
        cells_to_search = [
            tuple(int(j) for j in i.split(","))
            for i in n_indeces.difference(cells_coords_set)
        ]
        neighbours_space_types = [
            get_space_type(layout, i, j) for (i, j) in cells_to_search
        ]
